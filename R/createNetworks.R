...
#'   column names if using undirected networks, e.g. "XA_B" and "XB_A".
#' @param directed A logical variable indicating whether the networks
#'   being generated are directed. The default is set to FALSE.
createNetworks <- function(edge.df, directed = FALSE) {
  
  # make sure entered values are of the right type
  if(class(edge.df) != "data.frame" || class(directed) != "logical")
